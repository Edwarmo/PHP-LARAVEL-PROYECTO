...
onMounted(async () => {
  ...
  ctx = gsap.context(() => {
    // ── Partículas: cada una flota de forma independiente ──
    ...
    // ── Líneas: aparecen y desaparecen escalonadas ──
...
<template>
  <div class="fixed inset-0 -z-10 overflow-hidden pointer-events-none select-none">
    ...
    <!-- Orbes de luz atmosférica -->
    <div class="orb-a absolute rounded-full" style="
      width:700px; height:700px; top:-180px; left:-120px;
      background: radial-gradient(circle, rgba(0,200,255,0.08) 0%, transparent 70%);
      filter: blur(50px);"></div>
    <div class="orb-b absolute rounded-full" style="
      width:800px; height:800px; bottom:-250px; right:-180px;
      background: radial-gradient(circle, rgba(180,255,0,0.06) 0%, transparent 70%);
      filter: blur(70px);"></div>
    <div class="orb-c absolute rounded-full" style="
      width:500px; height:500px; top:45%; left:45%; transform:translate(-50%,-50%);
      background: radial-gradient(circle, rgba(0,180,255,0.05) 0%, transparent 70%);
      filter: blur(35px);"></div>

    <!-- Grid sutil -->
    <div class="gsap-grid absolute inset-0" style="
      opacity: 0.055;
      background-image:
        linear-gradient(rgba(0,220,255,0.7) 1px, transparent 1px),
        linear-gradient(90deg, rgba(0,220,255,0.7) 1px, transparent 1px);
      background-size: 60px 60px;"></div>

    <!-- SVG líneas y partículas -->
    <svg class="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <radialGradient id="rg-c" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   style="stop-color:#00dcff;stop-opacity:1"/>
          <stop offset="100%" style="stop-color:#00dcff;stop-opacity:0"/>
        </radialGradient>
        <radialGradient id="rg-l" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   style="stop-color:#c8ff00;stop-opacity:1"/>
          <stop offset="100%" style="stop-color:#c8ff00;stop-opacity:0"/>
        </radialGradient>
        <filter id="gf">
